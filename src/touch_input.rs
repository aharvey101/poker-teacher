...
use crate::betting_ui::{BettingButton, BettingButtonAction, HumanPlayerInput};
...
pub fn handle_unified_input(
    ...
    for (betting_button, interaction) in &interaction_query {
        if matches!(*interaction, Interaction::Pressed) {
            ...
            handle_betting_action(
                &mut human_input,
                &betting_button.action,
                &mut haptic_feedback,
            );
        }
    }
...
fn handle_touch_started(
    ...
    for (node, transform, betting_button) in all_button_query.iter() {
        ...
        info!(
            "Checking button {:?} at rect: {:?}, expanded: {:?}, touch at: {:?}",
...
pub fn handle_gesture_controls(
    ...
    for event in touch_events.read() {
        match event.phase {
            TouchPhase::Started => unsafe {
                GESTURE_START = Some(event.position);
            },
            TouchPhase::Ended => {
                if let Some(start_pos) = unsafe { GESTURE_START } {
                    ...
                    if swipe_distance.x.abs() > unsafe { GESTURE_THRESHOLD }
                        && swipe_distance.y.abs() < 30.0
                    {
                        if swipe_distance.x > 0.0 {
                            ...
                            human_input.raise_amount =
                                (human_input.raise_amount.saturating_sub(10)).max(5);
                            info!("Gesture: Decreased raise to {}", human_input.raise_amount);
                        }
                    }

                    // Vertical swipes for quick actions
                    if swipe_distance.y.abs() > unsafe { GESTURE_THRESHOLD }
                        && swipe_distance.x.abs() < 30.0
                    {
