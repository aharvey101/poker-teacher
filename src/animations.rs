...
impl Plugin for AnimationPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(
            Update,
            (animate_cards, animate_chips, cleanup_finished_animations),
        );
    }
}

// System to animate card movements
fn animate_cards(time: Res<Time>, mut query: Query<(&mut Transform, &mut CardAnimation)>) {
...
fn animate_chips(time: Res<Time>, mut query: Query<(&mut Transform, &mut ChipAnimation)>) {
    for (mut transform, mut animation) in query.iter_mut() {
        animation.progress += time.delta_seconds() / animation.duration;
