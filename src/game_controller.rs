    ...
    if controller.state_timer.finished() {
        match current_state.get() {
            ...
            GameState::Flop => {
                ...
                info!(
                    "Flop dealt: {} community cards",
                    game_data.community_cards.len()
                );
            ...
            GameState::GameOver => {
                ...
                let players_with_chips: Vec<&Player> =
                    players.iter().filter(|p| p.chips > 0).collect();
                ...
                if players_with_chips.len() <= 1 {
                    ...
                    if let Some(winner) = players_with_chips.first() {
                        ...
                        info!(
                            "🎉 GAME OVER! {} Player {} wins the entire game with ${} chips!",
                    ...
                    for player in &players_with_chips {
                        ...
                        info!(
                            "  {} Player {}: ${} chips",
                            player_name, player.id, player.chips
                        );
                    }
...
fn determine_winner(
    ...
    game_data: &GameData,
    game_position: &mut GamePosition,
) {
    ...
    for mut player in players.iter_mut() {
        if player.id == *winner_id {
            ...
            info!(
                ...
                winner_name, winner_id, game_data.pot, player.chips
            );
            break;
        }
    }
...
pub fn toggle_auto_advance(input: Res<Input<KeyCode>>, mut controller: ResMut<GameController>) {
...
fn post_blinds(
    ...
    for mut player in players.iter_mut() {
        if player.id == small_blind_player {
            ...
            info!(
                "🔸 Player {} posts small blind: {} chips (remaining: {})",
    ...
    for mut player in players.iter_mut() {
        if player.id == big_blind_player {
            ...
            game_data.pot += blind_amount;
            info!(
                "🔹 Player {} posts big blind: {} chips (remaining: {})",
                player.id, blind_amount, player.chips
            );
            break;
        }
