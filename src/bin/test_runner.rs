...
fn test_compilation() {
    ...
    let debug_result = Command::new("cargo").args(["build"]).output();
    ...
    let release_result = Command::new("cargo").args(["build", "--release"]).output();
...
fn test_game_startup() {
    ...
    match child {
        Ok(mut process) => {
            ...
            match process.kill() {
                Ok(_) => {
                    println!("✅ Game started successfully and was terminated after 10 seconds!")
                }
...
fn generate_test_report() {
    ...
    // Count Rust files
    let file_count_result = Command::new("find").args(["src", "-name", "*.rs"]).output();
